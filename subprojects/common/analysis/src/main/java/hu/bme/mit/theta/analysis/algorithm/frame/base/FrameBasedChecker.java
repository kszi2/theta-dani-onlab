...
public abstract class FrameBasedChecker<O extends BaseOptimizations>
    ...
    protected Cube removeRedundantExpressionsUsingUnsatCore(
        ...
        for (Expr<BoolType> expr : expressions.getLiterals()) {
            if (!unSatCore.contains(PathUtils.unfold(expr, monolithicExpr.getTransOffsetIndex()))) {
    ...
    protected @Nullable LinkedList<ProofObligation> checkIfFaultyReachableInOneStep() {
        final Valuation model =
                frames.getFirst()
                        .checkIfTargetIsReachableValuation(Not(monolithicExpr.getPropExpr()));
    ...
    protected LinkedList<ProofObligation> checkIfFaultyIntersectsInit() {
        Valuation model =
                frames.getFirst().checkIfContainsValuation(Not(monolithicExpr.getPropExpr()));
        if (model != null) {
    ...
    protected ProofObligation checkCurrentFrameForInterSections(Expr<BoolType> target) {
        ...
        if (optimizations.isPropertyOpt()) {
            interSection =
                    convertValuationToExpression(
                            frames.get(currentFrameNumber)
                                    .checkIfTargetIsReachableValuation(target));
        } else {
            interSection =
                    convertValuationToExpression(
                            frames.get(currentFrameNumber).checkIfContainsValuation(target));
        }
    ...
    protected Cube generalizeIter(
            Cube blockedCube, int currentFrameNumber, boolean canIntersectInit) {
        boolean done = false;
        boolean isSat;
        ...
        while (!done) {
            ...
            for (Expr<BoolType> expr : firstCopiedExpressions) {
                ...
                if (!canIntersectInit
                        && checkIfExpressionIntersectsInit(minimalCube.getLiterals())) {
